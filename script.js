...
function layoutAlignedEmojisDesktop(animate = true) {
    ...
    const zoomOutLevel = Math.min(availableWidth / totalWidth, 1.0);
    
    // Zoom IN: final zoom level (fits in LEFT 2/3 region)
    const requiredZoom = regionWidth / totalWidth;
    let bestIndex = 0;
    ...
    // Pan for zoom in (left-aligned)
    const screenLeftAtPan0 = (worldLeftEdge - centerX) * finalZoom + centerX;
    const finalPanX = regionLeft - screenLeftAtPan0;
...
function layoutAlignedEmojisMobileVertical(animate = true) {
    ...
    const paddingScreen = canvas.width * 0.10; // 10% left margin
    const targetWidthScreen = canvas.width * 0.60; // 60% width
...
const imagePaths = [
    ...
    'Imgae test /Addon 26 #instalation/IMG_3654.HEIC',
    'Imgae test /Addon 26 #instalation/IMG_3659.HEIC',
    ...
    'Imgae test /Addon 26 #instalation/Screenshot 2026-01-03 at 15.54.20.png',
    'Imgae test /Addon 26 #instalation/Screenshot 2026-01-03 at 15.56.36.png',
    'Imgae test /Bluebeard_s Castle #stage #installation /1f8621a766d563d6bbc3a36dbd1d04fa.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /46838.jpeg',
    'Imgae test /Bluebeard_s Castle #stage #installation /76769.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /789696 copy.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /7d0a27ca170dc646851659dac4ab005b.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /and-3210.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /e7af5fe7403946483739d3efbce015c9.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /photo_2022-11-20_14-41-10.jpg',
    'Imgae test /Bluebeard_s Castle #stage #installation /photo_2022-11-20_14-42-02.jpg',
    ...
    'Imgae test /New star camp #stage/fin4.png',
    'Imgae test /New star camp #stage/fin6.png',
    'Imgae test /Nina kravitz #stage/god_rays11.png',
    'Imgae test /Nina kravitz #stage/god_rays6.png',
    'Imgae test /Nina kravitz #stage/photo_2022-11-11_15-44-50.jpg',
    'Imgae test /Nina kravitz #stage/photo_2022-11-11_15-45-01.jpg',
    'Imgae test /Nina kravitz #stage/photo_2022-11-11_15-45-02.jpg',
    'Imgae test /Nina kravitz #stage/photo_2022-11-11_15-45-04.jpg',
    'Imgae test /Nina kravitz #stage/photo_2022-11-12_14-26-35.jpg',
    'Imgae test /Nina kravitz #stage/photo_2022-11-12_18-19-42.jpg',
    'Imgae test /Nina kravitz #stage/photo_3812@11-11-2022_13-45-04_thumb.jpg',
    'Imgae test /Nina kravitz #stage/photo_3814@12-11-2022_12-26-35.jpg',
    ...
    'Imgae test /belgium institution #spatial #installation/photo_2022-08-25_12-45-14.jpg',
    'Imgae test /belgium institution #spatial #installation/photo_2022-08-25_12-45-28.jpg',
    'Imgae test /belgium institution #spatial #installation/photo_2022-08-25_12-45-30.jpg',
    'Imgae test /belgium institution #spatial #installation/photo_2022-08-25_16-07-59.jpg',
    'Imgae test /belgium institution #spatial #installation/photo_2022-08-26_12-24-51.jpg',
    'Imgae test /bipolar express #stage #tech/IMG_2470.HEIC',
    ...
    'Imgae test /gula merah #stage/gula2.jpg',
    ...
    'Imgae test /port #stage/blue-lockers1.png',
    'Imgae test /port #stage/full_farsh copy.png',
    'Imgae test /port #stage/port - stage 3.jpg',
    'Imgae test /port #stage/port - stage 6.jpg',
    'Imgae test /port #stage/port - stage 7 .jpg',
    'Imgae test /port #stage/port - stage2.jpg',
    'Imgae test /port #stage/port stage 9 .jpg',
    'Imgae test /port #stage/port-stage 8 .jpg',
    'Imgae test /port #stage/port-stage.jpg',
    ...
    'Imgae test /tower building #spatial #installation/v2_smallCube_lights.png',
...
function checkIfReadyToShowImages() {
    ...
    allImagesLoaded = imagesLoaded >= totalImages && totalImages > 0;
...
function loadImageOnce(path) {
    return new Promise((resolve, reject) => {
        ...
        // Encode path to safely handle spaces/special chars, but avoid crossOrigin to prevent CORS blocks on mobile
...
// Generate points for ALL images (one point per image)
const points = generatePoints(imagePaths.length, 50);
...
function handleMouseDown(e) {
    ...
    // Allow navigation (dragging) when images are aligned, BUT allow clicking on other images to select them
    if (alignedEmojiIndex !== null) {
...
function createProjectAboutElements() {
    ...
    return { containerEl, nameEl, infoEl };
}
...
function loadAndDisplayAboutText(folderPath) {
    if (!folderPath) {
        ...
        displayProjectAboutText('~', []);
        }
        return;
    }
    
    // Encode folder path for URL (handle spaces and special characters)
    const encodedPath = folderPath.split('/').map(segment => encodeURIComponent(segment)).join('/');
    const aboutUrl = `${encodedPath}/about.txt`;
    ...
    fetch(aboutUrl)
        ...
        .then(text => {
            if (isMobileDevice()) {
                parseAndDisplayMobileAboutText(text);
            } else {
            parseAndDisplayAboutText(text);
            }
        })
        .catch(error => {
            if (isMobileDevice()) {
...
// Parse about.txt content for mobile display
function parseAndDisplayMobileAboutText(text) {
    ...
    // Extract about lines (project, client, studio, year, project status, contributor)
    const aboutLines = [];
    const labels = ['project', 'client', 'studio', 'year', 'project status', 'contributor'];
    
    labels.forEach(label => {
        const regex = new RegExp(`${label}:\\s*\\(([^)]+)\\)`, 'i');
        const match = aboutBlock.match(regex);
        if (match) {
            aboutLines.push({
                label: label,
                value: match[1].trim()
...
// Parse about.txt content
function parseAndDisplayAboutText(text) {
    if (!text || !text.trim()) {
        displayProjectAboutText('~', []);
        return;
    }
...
// Display project about text (desktop only - mobile uses displayMobileAboutText)
function displayProjectAboutText(name, aboutLines) {
    // On mobile, hide the desktop-style project about text
    if (isMobileDevice()) {
    ...
    const { containerEl, nameEl, infoEl } = createProjectAboutElements();
    
    // Set name
    nameEl.textContent = name || '~';
    
    // Clear and populate info lines
    infoEl.innerHTML = '';
    aboutLines.forEach((line, index) => {
        const lineEl = document.createElement('div');
    ...
    containerEl.style.opacity = '1'; // Force opacity for mobile
...
function updateProjectAboutTextPosition(containerEl, nameEl, infoEl) {
    ...
    const firstImageTopScreenY = ((firstImageTopY - centerY) * zoom) + centerY + cameraPanY;
    const firstImageLeftScreenX = ((firstImageLeftX - centerX) * zoom) + centerX + cameraPanX;
    const firstImageRightScreenX = ((firstImageRightX - centerX) * zoom) + centerX + cameraPanX;
    ...
    const lastImageBottomScreenY = ((lastImageBottomY - centerY) * zoom) + centerY + cameraPanY;
    ...
    if (isMobile) {
        ...
        // Desktop: Position name: 25px above top edge of first image, aligned to left edge
        nameEl.style.position = 'fixed';
        nameEl.style.left = `${firstImageLeftScreenX}px`;
        nameEl.style.right = 'auto';
        nameEl.style.top = `${firstImageTopScreenY - 25}px`;
        nameEl.style.textAlign = 'left';
        
        // Position info: 15px below bottom edge of last image, aligned to left edge
        infoEl.style.position = 'fixed';
        infoEl.style.left = `${firstImageLeftScreenX}px`;
        infoEl.style.right = 'auto';
        infoEl.style.top = `${lastImageBottomScreenY + 15}px`;
        infoEl.style.textAlign = 'left';
        
        containerEl.style.position = 'fixed';
        containerEl.style.left = `${firstImageLeftScreenX}px`;
...
window.addEventListener('resize', () => {
    ...
    const newPoints = generatePoints(imagePaths.length, 50);
    points.length = 0;
